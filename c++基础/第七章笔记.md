...
​	对于一个类来说，在我们创建它的对象之前该类必须被定义过，而不仅仅是声明。否则，编译器就无法了解这样的对象需要多少存储空间。类似的，类也必须被定义，然后才能用引用或者指针访问其成员。因为只有当类全部完成后类才算被定义，**所以一个类的成员不能是他自己。但是类允许包含指向它自身的引用或者指针。**

​	**类声明后可以定义类的指针或引用，但是不能访问元素。类定义后可以定义类的
